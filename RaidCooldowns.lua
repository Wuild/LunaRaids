...
function Raid:GetLocalRaidCooldownRemaining(definition)
    ...
    for _, spellID in ipairs(definition[6]) do
        ...
        if known then
            ...
            -- Ignore the global cooldown; only publish real ability timers.
        ...
        if settings.spells[definition[1]] ~= false then
            ...
            for _, player in ipairs(self.roster or {}) do
                if self:IsRaidCooldownPlayerEligible(player, definition) then
                    ...
                    -- Leave most players ready while putting a predictable
                    -- subset on cooldown so every visual state can be tested.
...
function Raid:InitializeRaidCooldowns()
    ...
    self.raidCooldownState = self:GetRaidCooldownSettings().active
    self:RegisterEvent(
        "COMBAT_LOG_EVENT_UNFILTERED", "HandleRaidCooldownCombatLog")
    self:RegisterEvent(
        "SPELL_UPDATE_COOLDOWN", "HandleLocalRaidCooldownUpdate")
